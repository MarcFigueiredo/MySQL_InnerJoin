...
/* Projecting all data */
SELECT CAR.IDCARRO, M.MARCA, CAR.MODELO, COR.COR, CAR.PLACA, CL.NOME, CL.SEXO, T.TIPO, T.NUMERO
FROM CARRO CAR
INNER JOIN CLIENTE CL
ON CAR.IDCARRO = CL.ID_CARRO
LEFT JOIN TELEFONE T
ON CL.IDCLIENTE = T.ID_CLIENTE
INNER JOIN MARCA M
ON CAR.ID_MARCA = M.IDMARCA
INNER JOIN CARRO_COR CC
ON CAR.IDCARRO = CC.ID_CARRO
INNER JOIN COR
ON CC.ID_COR = COR.IDCOR;

/* Projecting 'modelo' and 'placa' of red (vermelho) cars */
...
WHERE COR.COR = "VERMELHO";

/* Projecting 'marca' and 'nome'(driver) of each car */
...
/* Projecting 'nome'(driver) and 'marca' of cars from female drivers */
...
/* Projecting 'nome'(driver) and 'numero'(phone number) of yellow (amarela) cars */
SELECT NOME, NUMERO, COR
FROM CLIENTE
INNER JOIN TELEFONE
...
WHERE COR.COR = "AMARELA";

/* Projecting 'modelo'(car), 'nome'(driver) and 'sexo'(gender) of 'turquesa' or 'esmeralda' cars */
SELECT CAR.MODELO, CL.NOME, CL.SEXO, COR.COR
FROM CARRO CAR
INNER JOIN CLIENTE CL
ON CL.ID_CARRO = CAR.IDCARRO
INNER JOIN CARRO_COR CC
ON CC.ID_CARRO = CAR.IDCARRO
INNER JOIN COR
ON COR.IDCOR = CC.ID_COR
WHERE COR.COR = "TURQUESA" OR COR.COR = "ESMERALDA";